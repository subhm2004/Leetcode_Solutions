...
//         int n = fruits.size(), m = baskets.size(), used_count = 0;
        
//         for (int i = 0; i < n; ++i) {
//             for (int j = 0; j < m; ++j) {
//                 if (baskets[j] >= fruits[i]) {
//                     baskets[j] = INT_MIN; // Basket use ho chuki hai, mark as used
//                     ++used_count;        // Ek fruit place ho gaya
//                     break;         // Next fruit pe chalte hain
//                 }
//             }
//         }
        
//         return n - used_count; // Return the number of unplaced fruits
//     }
// };
...
public:
    int numOfUnplacedFruits(vector<int>& fruits, vector<int>& baskets) {
        ...
        for (int fruit : fruits) {
            // If max in whole range is less than fruit, skip
