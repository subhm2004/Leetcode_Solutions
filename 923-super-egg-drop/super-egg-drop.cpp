...
public:
    ...
    int solve_Binary_ME(vector<vector<int>>& dp, int K, int N) {
        ...
        while (l <= r) {
            ...
            int broken = solve_Binary_ME(dp, K - 1, mid - 1); // Egg breaks → check below
            int unbroken = solve_Binary_ME(dp, K, N - mid);   // Egg survives → check above
