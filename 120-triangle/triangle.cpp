...
public:
    ...
    int solve_bottom_up(vector<vector<int>>& arr) {
        int n = arr.size();  // Triangle ka size (rows ki count)
        ...
        */
        for (int i = n-2; i >= 0; i--) {
            ...
            */
            for (int j = 0; j <= i; j++) {  // j <= i kyunki triangle shape
                /*
                KEY INSIGHT:
                - dp[j] = previous iteration mein row (i+1), position j ka result
                - dp[j+1] = previous iteration mein row (i+1), position (j+1) ka result
