...
public:
    ...
    int solveRE_2(int i, int prev_parity, int target_parity, vector<int>& nums) {
        ...
        if (prev_parity == -1 || (prev_parity + curr_parity) % 2 == target_parity) {
            include = 1 + solveRE_2(i + 1, curr_parity, target_parity, nums);
        }
    ...
    int solveME_2(int i, int prev_parity, int target_parity, vector<int>& nums, vector<vector<int>>& dp) {
        ...
        if (prev_parity == -1 || (prev_parity + curr_parity) % 2 == target_parity) {
            include = 1 + solveME_2(i + 1, curr_parity, target_parity, nums, dp);
        }
    ...
    int solve_optmized(vector<int>& nums) {
