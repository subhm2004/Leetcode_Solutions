...
public:
    ...
    void dfs(int node, unordered_map<int, bool>& visited, int& nodes, int& edges) {
        visited[node] = true; // Is node ko visited mark kar rahe hain
        nodes++; // Component me ek aur node add ho gayi
        edges += adjList[node].size(); // Current node ke saare edges count kar rahe hain

        for (auto nbr : adjList[node]) { // Saare neighbours ko traverse karenge
            if (!visited[nbr]) { // Agar neighbour pehle visit nahi hua hai toh DFS call karenge
    ...
    int countCompleteComponents(int n, vector<vector<int>>& edges) {
        ...
        for (int i = 0; i < n; i++) {
            if (!visited[i]) { // Agar node abhi tak visit nahi hui hai toh naya component start hoga
                int nodes = 0, edges = 0;
                dfs(i, visited, nodes, edges); // DFS run karke component ka size aur edges nikal rahe hain

                // Ek complete graph ke liye, edges ki sankhya nodes * (nodes - 1) honi chahiye
                if (nodes * (nodes - 1) == edges) {
                    count++; // Agar condition match hoti hai toh count badhayenge
                }
            }
        }
        return count; // Final count return karenge
